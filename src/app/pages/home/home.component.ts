...
export class HomeComponent implements OnInit, AfterViewInit {
  // Arrival container slide variabl
  ...
  ngOnInit() {
    ...
    this.homeService.getArrivalCards().subscribe(cards => {
      console.log('Course Cards:', cards);
      this.courseCards = cards;
      setTimeout(() => this.updateDimensionsCourses(), 0);
    });

    this.homeService.getRecommendedCards().subscribe(cards => {
      console.log('Recommended Cards:', cards);
      this.recommendedCards = cards;
      setTimeout(() => this.updateDimensionsRecommended(), 0);
    });
  }
